isoph=function(formula, data=NULL, maxiter=10^4, eps=10^-3){
  ...
  if(COV.TYPE=='time.indep'){
    ...
    if(P==0){
      ...
      res=isoph.ti(TIME=TIME, STATUS=STATUS, Z=Z, ZNAME=ZNAME, P=P, Q=Q, shape=shape, K=K, maxiter=maxiter, eps=eps)
      res$call=match.call()
      res$formula=formula
      class(res)="isoph"
    ...
    #res=isoph.td
    stop("time-depdent cov is not supported for the current version of the isoSurv package")
  }
  return(res)
}
